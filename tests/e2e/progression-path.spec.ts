...
test.describe('game tree — Hermetic rational path', () => {
  test('rational fork reveals rosicrucian marker and path flags', async ({ page }) => {
    ...
    await page.getByTestId('marker-grove-rosicrucian-visible').waitFor({ state: 'attached' });
...
test.describe('game tree — Hermetic experiential path', () => {
  test('experiential fork sets practice flag without rosicrucian marker', async ({ page }) => {
    ...
    await page.getByTestId('marker-grove-pythagorean-visible').waitFor({ state: 'attached' });
