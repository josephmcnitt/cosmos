...
test.describe('game tree — Hermetic rational path', () => {
  test('rational fork reveals rosicrucian marker and path flags', async ({ page }) => {
    ...
    const pythagoreanMarker = save.entities?.find(
      (e: { id: string; state: { progressRevealed?: boolean } }) => e.id === 'grove-pythagorean',
    );
    expect(pythagoreanMarker?.state.progressRevealed).toBe(false);
  });
