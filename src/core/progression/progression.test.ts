...
describe('applyProgressEffects', () => {
  ...
  it('reveals pythagorean marker on experiential branch', () => {
    const entities = spawnEntitiesForAge(GROVE_AGE);
    const initialMarker = entities.find((e) => e.id === 'grove-pythagorean');
    expect(initialMarker?.state.progressHidden).toBe(true);
    expect(initialMarker?.state.progressRevealed).toBe(false);
    ...
    expect(marker?.defId).toBe('pythagorean-mysteries');
    expect(marker?.state.progressRevealed).toBe(true);
  });
