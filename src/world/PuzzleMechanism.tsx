...
export function PuzzleMechanism({ entity }: { entity: EntityInstance }) {
  ...
  return (
    <group
      ref={ref}
      position={[entity.transform.x, y + 0.5, entity.transform.z]}
    >
      {rotations.map((rot, i) => (
        <mesh key={i} rotation={[0, (rot * Math.PI) / 2, 0]} position={[0, i * 0.35, 0]}>
          <torusGeometry args={[0.5 + i * 0.15, 0.04, 8, 24]} />
          <meshStandardMaterial color={i === 0 ? '#4ecdc4' : i === 1 ? '#c8a860' : '#b088f0'} />
        </mesh>
      ))}
    </group>
  );
